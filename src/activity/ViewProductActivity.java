...
public class ViewProductActivity extends JFrame implements ActionListener, MouseListener {
	...
	public ViewProductActivity(JFrame prev, Admin admin) {
		super("View Product");
		...
		keywordLabel = new JLabel("Keyword: ");
		keywordLabel.setBounds(60, 140, 1400, 30);
		keywordLabel.setFont(Theme.FONT_REGULAR);
		panel.add(keywordLabel);
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource().equals(buttonLogout)) {
			...
			this.setVisible(false);
			new AddProductActivity(this, admin).setVisible(true);
		}
		else {}
	}
	...
	private void jTable_ClickMouseClicked(MouseEvent evt) {                                          
       ...
       String value1 = model.getValueAt(index, 0).toString();
		if (customer!=null) {
		    ...
		    String unitPrice = model.getValueAt(index, 2).toString();
		    String quantityInStock = model.getValueAt(index, 3).toString();
		    buyproduct3 buyProduct = new buyproduct3(this, customer,productID, productName, unitPrice, quantityInStock);
//		    buyproduct3 buyProduct = new buyproduct3(productID, productName, unitPrice, quantityInStock);
